...
pub fn part1() {
    ...
    junction_box_circuits.sort_by_key(|k| 10000 - k.len());
        ...
        match (circuit1, circuit2) {
            (Some(c1), Some(c2)) => {
                if c1 != c2 {
                    ...
                    c2.clear(); // Just leave empty circuits
                    if c1.len() == junction_boxes.len() {
                        println!("Result: {}", box1.0 * box2.0);
                        break; // No need to go through any more pairs
                    }
                }
            }
            (Some(c1), None) => {
